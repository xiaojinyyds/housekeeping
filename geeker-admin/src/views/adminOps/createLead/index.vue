<template>
  <div class="page">
    <div class="card header-card">
      <div>
        <h2>新建客户线索</h2>
        <p>把 Excel 里的客户登记逐步迁到后台，统一给员工和老板查看。</p>
      </div>
...
import { createLeadApi } from "@/api/modules/business";
...
const submitForm = async () => {
  ...
  try {
    ...
    ElMessage.success("客户线索创建成功");
    router.push({ path: "/lead/list", query: { refresh: Date.now().toString() } });
  } finally {
